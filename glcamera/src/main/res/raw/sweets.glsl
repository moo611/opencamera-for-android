...
vec4 sharpen(samplerExternalOES sampler)
{ 
...
{ 
	...
	if (lowPerformance == 1) 
	{ 	
		c1 = texture2D(inputImageTexture, vTextureCoord); 	
		c2 = texture2D(inputImageTexture, vTextureCoord); 
    } 
	else 
	{ 
	...
	c3 = normal(c3, overlay(c3, vec4(0.)), 1.-texture2D(samplerMask, vTextureCoord).r); // vignetting 
